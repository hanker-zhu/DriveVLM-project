...
def main(args):
    ...
    model_c.eval()
    model_c.qconfig = torch.quantization.get_default_qconfig('fbgemm')
    torch.quantization.prepare(model_c)
    qmodel = torch.quantization.convert(model_c)
    qmodel.to(args.device)

    model.to(args.device)
    generation_config = GenerationConfig.from_pretrained("lykong/paligemma-finetuned")
